...
<form action="remove.php" method="get">
    ...
    <select name="empid" id="selection">
        ...
        $ids = mysqli_query($con, "select d.docId, e.firstName, e.lastName from doctor as d, employee as e where d.docId=e.employeeId");

        while ($row = mysqli_fetch_array($ids))
            {
                echo "<option value='$row[docId]'>$row[docId] - $row[firstName] $row[lastName]</option>";
            }
        ?>
    </select>
